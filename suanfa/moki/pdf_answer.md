...
class simple_balance(object):
    ...
    def GET(self, r_kind=0):
        """ 假定，我们给出的 request-x 与 backend-y 的 f(x), 
            简单的是： x = y，这样就不是硬编码了 """
...
### 2. 问答题：
> 前提条件：
> - 至少5分钟刷新一次cache内容，并不严格要求时间.
> - backend生成内容10秒钟左右，并不严格要求时间。

#### 1. 解决方案1 -- 这是我一开始想到的方法：
> - 核心想法：利用 异步请求，cache 主动 *提前*通知 backend生成内容，cache不必再等待 backend.
> - *ps: 我在纸上，画了一个不太标准的时序图，大致能说明我的想法* 
> - *如果没有说明白，还是需要口头交流一下。我一开始想得稍微复杂，就画时序图说明吧*
...
> *你好，我是睡了一觉之后的想法1. 这个想法实现应该比较自然，celery+redis就可以.*

#### 3. 解决方案3：
> - 核心想法：利用 crontab，cache并不主动从backend获取内容，backend每隔2分钟，主动生成一次新内容，放到redis中

> *你好，我是睡了一觉之后的想法2. 这个实现起来方法最快, 我平时会用这个方法*
